...
class CONTEXT_MENU {
    constructor(INTERACTIONS_MANAGER){
        ...
        this.actions = {
            openInNewTab: (parentElement) => {
                window.open(parentElement.parentNode.querySelector("[data-url]").dataset.url);
                this.actions.closeMenu(parentElement, target)
            },
            editShortcut: async (parentElement) => {
                this.actions.closeMenu(parentElement);
                const currentShortcut = parentElement.parentNode.querySelector(".shortcutBody");
                const currentShortcutURL = currentShortcut.querySelector("legend").dataset.url;
                const currentShortcutName =  currentShortcut.querySelector("legend").textContent;
                document.getElementById("root").style.cursor = "progress";
                let shortcutPrompt = (await import("./editShortcutPrompt.js")).shortcutPrompt;
                document.getElementById("root").style.cursor = "auto";
                shortcutPrompt.openMenu({lang: this.lang, mode: 1, currentShortcut: currentShortcut, title: this.lang.prompts.shortcuts.editTitle, URLplaceholder: currentShortcutURL, NamePlaceholder: currentShortcutName})
            },
    ...
    insertMenu(container, lang) {
        ...
        if(this.openedMenu === true) {
            this.currentParent.parentNode.removeChild(document.getElementById("contextMenu"));
            this.openedMenu = false;
        }
        this.style = `
            #contextMenu {
        ...
        document.querySelector("head").insertAdjacentHTML("beforeend", `<style id="contextMenuStyle">${this.style}</style>`);
        const element = document.getElementById("contextMenu");
        const elementRect = element.getBoundingClientRect();
        let yTranslate = 0;
        let xTranslate = elementRect.width*0.10;
        if((window.innerHeight - elementRect.bottom) < elementRect.height*0.10) yTranslate = elementRect.height*0.15;
        if((window.innerWidth - elementRect.right) < elementRect.width) {
...
function interactionsManager(e){
    if(contextMenu.actions[e.target.dataset.action]){
        contextMenu.actions[e.target.dataset.action](e.target.parentNode.parentNode.parentNode, e.target);
    } else{
        if(e.target.dataset.action === '2') return
        contextMenu.actions.closeMenu(document.getElementById("contextMenu").parentNode)
    }
}
